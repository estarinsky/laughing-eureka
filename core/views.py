...
def type_the_word_game(request):
    ...
    # Sort all words by their score in ascending order
    all_words.sort(key=lambda w: w.score)

    # Define the priority pool (e.g., the 40% of words with the lowest scores)
    ...
    # 70% chance to pick a word from the priority (low-score) pool
    ...
    # 30% chance to pick from the other words
    else:
        if other_words:
            word_to_guess = random.choice(other_words)
    ...
    # A final fallback to ensure a word is always selected
